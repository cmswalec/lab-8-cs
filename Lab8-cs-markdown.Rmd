...
```{r}
spill_sp <- as(oil_sf, "Spatial") #converted from data fram to simple feature
spill_ppp <- as(spill_sp, "ppp")

tx_sp <- as(tex_border, "Spatial")
tx_owin <- as(tx_sp, "owin") #makes an o window
...
```{r}
oil_qt <- quadrat.test(all_ppp, nx = 5, ny = 5) #nx and ny are the number of regions in the x and y directions, respectively
oil_qt #reject null hypothesis that data are csr. In otherwords, conclude that data are csr

plot(all_ppp) #crosses represent "illegal" points
...
ggplot(oil_gfun, aes(x = r, y = obs)) + 
  ...
  geom_line(aes(x = r, y = theo), color = "red") #our data has a higher proportion of point data with closer nearest neighbors thancsr data would
```
...
#takeaway is still that there are higher densities of neighbors at small distances than there are for csr data
```
